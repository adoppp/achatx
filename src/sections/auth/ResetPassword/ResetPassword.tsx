...
export const ResetPassword: FC = () => {
    const { email, emailError, isLoading, handleChange, handleSubmit } = useResetPassword();
    ...
    return (
        <div className={cn('resetpassword')}>
            <div className={cn('resetpassword__container')}>
                <Button
                    customClassName={cn('resetpassword__back')}
                    variant="secondary"
                    size="s"
                ...
                <form className={cn('resetpassword__form')} onSubmit={handleSubmit}>
                    <Input label="Email" value={email} onChange={handleChange} error={emailError} />
                    <Button
                        customClassName={cn('resetpassword__button')}
                        isLoading={isLoading}
                        disabled={emailError !== null || email.trim() === ''}
                    >
                        Send reset email
                    </Button>
                </form>
