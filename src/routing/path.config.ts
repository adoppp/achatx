...
export const ABSOLUTE_URLS = {
    auth: {
        signIn: () => `${FRONTEND_URL}${ROUTE_URLS.auth.signIn().slice(6)}`,
        signUp: () => `${FRONTEND_URL}${ROUTE_URLS.auth.signUp().slice(6)}`,
        resetPassword: () => `${FRONTEND_URL}${ROUTE_URLS.auth.resetPassword().slice(6)}`,
    },
    app: {
        chats: () => `${FRONTEND_URL}${ROUTE_URLS.app.chats().slice(6)}`,
        chat: (id: number) => `${FRONTEND_URL}${ROUTE_URLS.app.chat(id).slice(6)}`,
        settings: () => `${FRONTEND_URL}${ROUTE_URLS.app.settings().slice(6)}`,
        profile: () => `${FRONTEND_URL}${ROUTE_URLS.app.profile().slice(6)}`
    }
} as const;